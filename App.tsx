...
import { JCardConfig, TransportState, TapeSide, Track } from './types';
...
import { fetchCCMixterTracks } from './utils/ccmixterUtils';
...
export default function App() {
  ...
  useEffect(() => {
    ...
    if (mixtapeData) {
      ...
      if (decoded) setJCardConfig(decoded);
        ...
        // Fetch some music from ccmixter
        fetchCCMixterTracks('instrumental').then(tracks => {
            setJCardConfig(prev => ({
                ...prev,
                tracklist: {
                    sideA: tracks.slice(0, 10),
                    sideB: tracks.slice(10, 20)
                }
            }));
        });
    }
  ...
  useEffect(() => {
    const tracks = transport.currentSide === 'A' ? jCardConfig.tracklist.sideA : jCardConfig.tracklist.sideB;
    if (tracks && transport.currentTrack < tracks.length) {
        setCurrentTrack(tracks[transport.currentTrack]);
    } else {
        setCurrentTrack(null);
    }
  }, [transport.currentSide, transport.currentTrack, jCardConfig.tracklist]);
  ...
  return (
    <div className={`min-h-screen bg-[#0a0a0a] flex items-center justify-center p-4 text-white font-sans relative overflow-hidden transition-all duration-500 ease-in-out ${showLinerNotes ? 'pr-[400px]' : ''}`}>
      <div className="w-full h-full flex flex-col items-center justify-center">
          ...
          <div className="w-full max-w-xl py-8">
            <UnifiedDeck
              ...
              onFlip={handleFlip}
              onOpenEditor={() => setShowLinerNotes(!showLinerNotes)}
              canFlip={canFlip}
            />
          </div>
          ...
          <AudioPlayer
            ...
            onEnded={() => {
                setTransport(prev => {
                    const tracks = prev.currentSide === 'A' ? jCardConfig.tracklist.sideA : jCardConfig.tracklist.sideB;
                    if (prev.currentTrack < tracks.length - 1) {
                        return { ...prev, currentTrack: prev.currentTrack + 1 };
                    }
